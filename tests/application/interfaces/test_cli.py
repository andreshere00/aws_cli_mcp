...
def test_build_client(monkeypatch, tools_file_dict):
    ...
    monkeypatch.setitem(
        sys.modules, "src.infrastructure.llm", SimpleNamespace(AzureOpenAIUnitOfWork=StubUoW)
    )
    monkeypatch.setitem(
        sys.modules, "src.infrastructure.mcp", SimpleNamespace(MCPClient=StubClient)
    )
...
def test_cmd_chat_interactive_loop_one_turn(monkeypatch, capsys, tools_file_dict):
    ...
    # Simulate a single user input then KeyboardInterrupt to exit the loop
    def input_once_then_kb(prompt="> "):
        if not hasattr(input_once_then_kb, "_called"):
            input_once_then_kb._called = True
            return "hello"
        raise KeyboardInterrupt()

    monkeypatch.setattr("builtins.input", input_once_then_kb)
